...
class KkDataSourcesSpider(scrapy.Spider):
    ...
    def parse(self, response):  # resonse相当于从网络中返回内容所存储的或对应的对象
        ...
        # next_href = response.xpath("//div[@class='page th']/a[text()='%d')]/@href" % str(int(this_page)+1)).extract()
        next_href = response.xpath("//div[@class='page th']/a[text() = '%s']/@href"% str(int(this_page)+1)).extract()[0].replace("/Article/media/economist/","")
        self.log('Saved file %s.')  # self.log是运行日志，不是必要的
        if next_href is not None:  # 判断是否存在下一页
            next_page = response.urljoin(KkDataSourcesSpider.start_urls[0]+next_href)
